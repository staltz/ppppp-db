...
function initDB(peer, config) {
  ...
  function getSigkeysInAccount(accountTangle, accountTipsInMsg) {
    console.log('accountTangle', accountTangle?.debug())
    // TODO: add test with more than 1 msg in the account tangle
    //getSigkeysInAccount needs a new argument: accountTips of type Msg['metadata']['accountTips']
    //use tangle.slice()
    const sigkeys = new Set()
    if (!accountTangle) return sigkeys

    const prunedTangle = accountTangle
      .slice(undefined, accountTipsInMsg ?? undefined)
      .map((msg) => MsgV4.getMsgID(msg))
    console.log('prunedTangle', prunedTangle)

    for (const msgID of prunedTangle) {
      const msg = get(msgID)
