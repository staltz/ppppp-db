    ...
    if (accountID) {
      DBTangle.init(accountID, records(), get).then((accountTangle) => {
        ...
        if (!accountTangle.has(accountID)) {
          return cb(Error(`Account tangle "${accountID}" is locally unknown`))
        }
          ...
          (err, sigkeys) => {
            ...
            if (typeof rec.msg.data === 'string') {
              ...
              if (MsgV4.isMsg(recDecrypted.msg.data)) {
                ...
                try {
                  verifyRec(innerRec, innerMsgID, cb)
                } catch (err) {
                  return cb(Error('Failed to verify inner msg', { cause: err }))
                }
              }
            }
        ...
        pull.drain(
          (msgID) => {
            ...
            get(msgID, (err, msg) => {
              ...
              if (err) {
              cbd = true
              return cb(Error("db.account.has() failed to get() account tangle message", { cause: err }))
            }
              if (!msg?.data) return
              /** @type {AccountData} */
              const data = msg.data
              if (data.action !== 'add') return
              if (data.key.algorithm !== keypair.curve) return
  ...
  function getAccountPowers(accountTangle, keypair, cb) {
    ...
    for (const msgID of accountTangle.topoSort()) {
      ...
      get(msgID, (err, msg) => {
        ...
        if (data.action !== 'add') return
        if (data.key.algorithm !== keypair.curve) return
        if (data.key.bytes !== keypair.public) return
        if (data.powers) {
  ...
  function addToAccount(opts, cb) {
    ...
    DBTangle.init(opts.account, records(), get).then((accountTangle) => {
      ...
      get(opts.account, (err, accountRoot) => {
        if (err)
          return cb(
            Error('account.add() failed to get() account root', { cause: err })
          )
        if (!accountRoot) {
          // prettier-ignore
  ...
  function delFromAccount(opts, cb) {
    ...
    DBTangle.init(opts.account, records(), get).then((accountTangle) => {
      ...
      get(opts.account, (err, accountRoot) => {
        if (err)
          return cb(
            Error('account.del() failed to get() account root', { cause: err })
          )
        if (!accountRoot) {
          // prettier-ignore
